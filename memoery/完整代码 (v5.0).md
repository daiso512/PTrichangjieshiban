    ...
    with tab1:
        ...
        if not df.empty:
            ...
            for index, row in df.iterrows():
                ...
                # 2. 生成标题字符串 (日期 | 图标 级别 | 分类 | 责任人)
                # 这种格式让用户不点开也能看到关键元数据
                label_text = f"{row['date']}  |  {attr_icon} {row['attribute']}  |  📂 {row['category']}  |  👤 {row['person']}  (ID: {row['id']})"
                
                # 3. 创建折叠区
                with st.expander(label_text):
                    # A. 显示完整文本内容
                    st.markdown(f"**{L['form_content']}:**")
                    st.info(row['content']) # 使用蓝色框展示内容，更显眼
